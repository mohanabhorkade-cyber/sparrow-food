...
@Component({
  ...
  styleUrl: './seasoning.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SeasoningComponent implements OnInit, OnDestroy {
  products: Product[] = [];
  ...
  ngOnInit(): void {
    this.http
      ...
      .subscribe({
        next: (data) => {
          this.products = data.filter((product) => product.category === 'seasonings');
          this.cdr.markForCheck();
        },
        error: (err) => console.error('Failed to load seasonings:', err)
      });
  }
